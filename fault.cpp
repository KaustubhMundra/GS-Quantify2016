...
list <inputList> inputTaken; 		//input list maintained dynamically
...
public:
	...
	{
		list <job>::iterator it;
		for(it=assignJobs.begin();it!=assignJobs.end();++it)				//finds the jobs completed from prev assignJobs list and removes the completed jobs
		{
		...
		minAssign=getmin(freeCPU,tStamp,k);
	...
	void query(unsigned long long int tStamp,unsigned long long int k)
	{
		...
		for(it=inputTaken.begin();it!=inputTaken.end();++it)		//iterates over the input list till the latest query call and removes the inputs traversed to save memory
		{
			...
			else			//indicates stored input as assign
			{
				...
				{
					list <job>::iterator it2;
					for(it2=queryAssignList.begin();it2!=queryAssignList.end();++it2)    //basic assign step as done earlier in assign
					{
						if((*it2).giveCompletionTime()<=(*it).assignTimestamp)
						{
							queryFreeCPU++;
							it2=queryAssignList.erase(it2);
							it2--;
						}
					}
					unsigned long long int minAssign;
					minAssign=getmin(queryFreeCPU,(*it).assignTimestamp,(*it).assignK);
	...
	{
		list <inputList> :: iterator it;
		for(it=inputTaken2.begin();it!=inputTaken2.end();++it)
		{
			if((*it).value ==1 )
			{
			...
			else
			{
				...
				{
					list <job>::iterator it2;
					for(it2=queryAssignList2.begin();it2!=queryAssignList2.end();++it2)
					{
						if((*it2).giveCompletionTime()<=(*it).assignTimestamp)
						{
							queryFreeCPU2++;
							it2=queryAssignList2.erase(it2);
							it2--;
						}
					}
					unsigned long long int minAssign;
					minAssign=getmin(queryFreeCPU2,(*it).assignTimestamp,(*it).assignK);
					list <job>:: iterator it3;
					list <job>:: iterator it4;
	...
	{
		list <job> :: iterator it;
		for(it=queryList2.begin();it!=queryList2.end();++it)
		{
			if((*it).giveOrig().compare(orig)==0)
			{
				(*it).display();
				cout<<endl;
			}
