...
public:
	...
	void query(unsigned long long int tStamp,unsigned long long int k)
	{
		...
		for(it=inputTaken.begin();it!=inputTaken.end();++it)		//iterates over the input list till the latest query call and removes the inputs traversed to save memory
		{
			...
			else			//indicates stored input as assign
			{
				...
				{
					list <job>::iterator it1;
					for(it1=queryAssignList.begin();it1!=queryAssignList.end();++it1)    //basic assign step as done earlier in assign
					{
	...
	{
		vector <inputList> :: iterator it;
		for(it=inputTaken.begin();it!=inputTaken.end();++it)
		{
			if((*it).value ==1 )
			{
			...
			else
			{
				...
				{
					list <job>::iterator it1;
					for(it1=queryAssignList2.begin();it1!=queryAssignList2.end();++it1)
					{
						if((*it1).giveCompletionTime()<=(*it).assignTimestamp)
						{
							queryFreeCPU2++;
